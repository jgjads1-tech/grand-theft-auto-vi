$(document).ready(function() {
    ...
    $(".m-ambient-video").each(function(index) {
        ...
        var iOSMobileVideoOverride = $(this).hasClass("pp-iOS-MobileVideo"); /*Class to add if you have a mobile video*/
        ...
        if (!video.controls) {
            ...
            if ($(this).hasClass("pp-button")) {
                ...
                if (isPlaying(video) || video.autoplay || iOSMobileVideoOverride) {
                ...
                if (!isIOS || iOSMobileVideoOverride) {
...
// character rotator pivot flipper positioning
    $(window).on('load resize', function() {
        if ($('.characterRotate .c-pivot').length > 0) {
            $('.characterRotate .c-pivot').each(function(){
                tablist = $(this).find("[role=tablist]").get(0);
                tablistHeight = tablist.offsetHeight;
                tablistTop = tablist.offsetTop;
                flipperHeight = $(this).find(".c-flipper").height();
                flipperBump = tablistTop + ((tablistHeight - flipperHeight) / 2);
                $(this).find(".c-flipper").css("top", flipperBump);
            });
        }
    });
    ...
    var popFaqStructuredData = (function() {
        ...
        for (var i = 1; i <= qnum; i++) {
            if ($("[aria-controls='row-" + i + "'] p").length !== 0) {
                var theQ = $("[aria-controls='row-" + i + "'] p").html();
                if (theQ !== undefined) {
                    theQ.replace(/"/g, "'");
                }
            } else {
                var theQ = $("[aria-controls='row-" + i + "'] h3").html();
                if (theQ !== undefined) {
                    theQ.replace(/"/g, "'");
                }
            }
...
locStrings = {
    "locales": {
        ...
        "zh-cn": {
            "keyPlay": "播放",
            "keyPause": "暂停播放",
            "keyPlaypressed": "按钮已启用",
            "keyPausepressed": "暂停按钮已启用"
        }
    }
}